...
// ─── SVG Icon Library ─────────────────────────────────────────────────────────
// All icons are inline SVG strings. Use `dangerouslySetInnerHTML` or a
// helper component to render them. Each is 20×20 viewBox for consistency.
...
// ─── 8-Level Rank Ladder ──────────────────────────────────────────────────────
// Balanced curve: fast start, meaningful mid-game, epic endgame.
// A user who books once + posts daily reaches Voyager in ~5–7 days.
// Legend requires sustained multi-month engagement.
...
export const XP_MULTIPLIERS = {
  free: 1,
  pro: 1.5, // Sky Pro:  1.5× — meaningful but not crushing for free users
  elite: 2.5, // Sky Elite: 2.5× all XP
};
...
export const XP_RULES = {
  ...
  SAVED_TRIP: {
    ...
    label: "Saved a trip",
    iconKey: "bookmark",
    dailyLimit: 5, // prevents spamming save/unsave
  },
  ...
  POST_CREATED: {
    ...
    iconKey: "broadcast",
    dailyLimit: 10, // up to +100 XP/day from posting
  },
  COMMENT_CREATED: {
    xp: 5,
    label: "Commented",
    iconKey: "chat",
    dailyLimit: 20, // up to +100 XP/day from commenting
  },
  POST_LIKED_RECEIVED: {
    xp: 2,
    label: "Your post got a like",
    iconKey: "star",
    dailyLimit: 25, // up to +50 XP/day from likes received
  },
  SHARE_SKYSTREAM: {
    xp: 8,
  ...
  PRO_MONTHLY_BONUS: {
    xp: 50,
    label: "Pro monthly XP bonus",
    iconKey: "star",
    // awarded by a monthly cron job for all pro members
  },
};

// ─── Guardrails ───────────────────────────────────────────────────────────────
export const XP_DAILY_CAP = 400; // soft launch guardrail (all sources combined)
export const XP_MAX_SINGLE_AWARD = 500; // single-award ceiling (admin can override)

// ─── Core Helper: getLevel ────────────────────────────────────────────────────
// Drop-in replacement for the old getTier/getLevel split.
// Returns everything a UI needs to render an XP badge or progress bar.

/**
 * @param {number} xp
 * @returns {{
 *   current: object,      // full XP_LEVELS entry
 *   next: object | null,
 *   xp: number,
 *   xpIntoLevel: number,
 *   xpNeeded: number,
 *   xpToNext: number,
 *   percent: number,
 *   icon: string,         // SVG string for current level
...
// ─── Legacy aliases (keep these so existing pages don't break) ────────────────
